...
def index():
    ...
    if request.method == 'POST':
        ...
        if payperiod_type == 'weekly':
            payperiod = Payperiod(is_weekly=True, is_biweekly=False, is_semi_monthly=False)
        elif payperiod_type == 'biweekly':
            payperiod = Payperiod(is_weekly=False, is_biweekly=True, is_semi_monthly=False)
        else:
            payperiod = Payperiod(is_weekly=False, is_biweekly=False, is_semi_monthly=True)
        ...
        if case == 'regular':
            ...
            flsa_instance = FLSA(total_reg_hours, total_ot_hours, total_worked_hours, base_hourly_rate, payperiod)
            result = {
            ...
            while True:
                ...
                i += 1
            flsa_instance = FLSA.from_alternate_rates(total_reg_hours, base_hourly_rate, alternate_rates, payperiod)
            result = {
